...
export const InputHandler = {
    init() {
        ...
        document.getElementById('chat-input').addEventListener('keydown', this.handleChat.bind(this));
        ...
        document.getElementById('gameCanvas').addEventListener('click', () => {
            ...
            } else if (Game.state === 'playing' && Game.localPlayer?.isAlive && UI.selectedSpellIndex !== null) {
        ...
        document.getElementById('opponents-grid').addEventListener('click', (e) => {
            if (Game.state === 'playing' && Game.localPlayer?.isAlive && UI.selectedSpellIndex !== null) {
                const opponentView = e.target.closest('.opponent-view');
                if (opponentView && !opponentView.classList.contains('empty-slot') && opponentView.id !== 'spell-announcement') {
    ...
    handleMouseMove(e) {
        if (!Game.localPlayer?.isAlive || Game.bubbleRadius === 0 || Game.state !== 'playing' || UI.selectedSpellIndex !== null) return;
        const mainCanvas = document.getElementById('gameCanvas');
        if (!mainCanvas) return;
        const rect = mainCanvas.getBoundingClientRect(); 
        const mouseX = e.clientX - rect.left, mouseY = e.clientY - rect.top;
        
        const launcherY = mainCanvas.height - Game.bubbleRadius; // Le pivot est maintenant en bas
        
        Game.localPlayer.launcher.angle = Math.atan2(mouseY - launcherY, mouseX - mainCanvas.width / 2);
    },
    handleShoot() {
        const p = Game.localPlayer; 
        if (Game.state !== 'playing' || !p?.isAlive || !p.launcherBubble || p.shotBubble || UI.selectedSpellIndex !== null) return; 
        
        const mainCanvas = document.getElementById('gameCanvas');
        if (!mainCanvas) return;
        
        p.shotBubble = p.launcherBubble; p.launcherBubble = null;
        ...
        const launcherY = mainCanvas.height - Game.bubbleRadius; // Point de départ du tir en bas
