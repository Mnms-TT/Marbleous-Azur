...
export const FirebaseController = {
  ...
  async init() {
    ...
    try {
      ...
      if (this.auth.currentUser) {
        ...
        throw new Error("Authentification anonyme échouée.");
      }
    } catch (error) {
  ...
  async joinGame() {
    ...
    const initialPlayerData = {
      name: `Joueur_${localPlayerId.substring(0, 4)}`,
      isAlive: true,
      isReady: false,
      team: 0,
  ...
  listenForGameChanges() {
    ...
    this.unsubscribePlayers = onSnapshot(playersCollection, (snapshot) => {
      ...
      for (const id of Game.players.keys())
        if (!serverIds.has(id)) Game.players.delete(id);
    ...
    this.unsubscribeGameSession = onSnapshot(sessionDoc, (doc) => {
      ...
      if (
        sessionData.gameState === "countdown" &&
  ...
  async updatePlayerDoc(playerId, data) {
    await setDoc(doc(this.db, "rooms", roomId, "players", playerId), data, {
      merge: true,
    });
  },
  async updateSessionDoc(data) {
    await setDoc(doc(this.db, "rooms", roomId), data, { merge: true });
  },
    ...
    try {
      await runTransaction(this.db, async (transaction) => {
        ...
        const newPlayerCount = Math.max(
          0,
          (roomDoc.data().playerCount || 1) - 1
        );
        transaction.delete(playerRef);
        transaction.update(roomRef, { playerCount: newPlayerCount });
      });
    } catch (e) {
      console.error("Erreur pour quitter la salle: ", e);
    }
  },
};
