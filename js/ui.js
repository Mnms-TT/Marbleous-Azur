...
export const UI = {
    ...
    renderOpponents() {
        ...
        const opponents = Array.from(Game.players.values()).filter(p => p.id !== FirebaseController.auth?.currentUser?.uid);
        
        const announcement = document.createElement('div');
        announcement.id = 'spell-announcement';
        announcement.className = 'opponent-view flex items-center justify-center text-center';
    ...
    renderTeamSelectionInAnnouncementBox() {
        ...
        Config.TEAM_COLORS.filter((_, i) => i !== Game.localPlayer.team).forEach(color => {
            const teamIndex = Config.TEAM_COLORS.indexOf(color);
            const btn = document.createElement('button');
            btn.className = 'team-choice-btn';
            btn.style.backgroundColor = color;
            btn.onclick = () => FirebaseController.updatePlayerDoc(FirebaseController.auth.currentUser.uid, { team: teamIndex });
    ...
    checkVoteStatus() {
        ...
        if (!Game.localPlayer.isReady) {
            ...
            this.updateSpellAnnouncement("Système", { name: `Prêts: ${ready} / ${required}` }, null);
        }

        Game.players.forEach(p => {
            const container = p.id === Game.localPlayer.id ? document.getElementById('canvas-container') : p.container;
            if (container) {
    ...
    startCountdown() {
        ...
        const slot = document.getElementById('spell-announcement'); if (!slot) return;
        let count = 3;
        const update = () => {
            if (count > 0) { slot.innerHTML = `<span class="text-4xl font-bold">${count}</span>`; count--; }
            else {
                slot.innerHTML = `<span class="text-2xl font-bold">GO!</span>`;
                clearInterval(Game.countdownInterval);
                if(Game.localPlayer) FirebaseController.updateSessionDoc({ gameState: 'playing' });
            }
        }; update(); Game.countdownInterval = setInterval(update, 1000);
    },
    stopCountdown() { if (Game.countdownInterval) { clearInterval(Game.countdownInterval); Game.countdownInterval = null; } },
    
    updatePlayerStats() {
        if (!Game.localPlayer) return;
        
        const teamIndicator = document.getElementById('main-player-team-indicator');
        if (teamIndicator) teamIndicator.style.backgroundColor = Config.TEAM_COLORS[Game.localPlayer.team];
        ...
        this.updateTargetingUI();
        Game.players.forEach(p => { if (p.id !== Game.localPlayer.id && p.teamIndicator) p.teamIndicator.style.backgroundColor = Config.TEAM_COLORS[p.team]; });
    },

    updateBoardEffects() {
        const cont = document.getElementById('canvas-container'); let transform = '';
        if (Game.localPlayer?.statusEffects.plateauIncline) transform += ` rotate(${Game.localPlayer.statusEffects.plateauIncline.direction * 7}deg)`;
        if (Game.shakeUntil && Date.now() < Game.shakeUntil) {
            const intensity = Game.shakeIntensity;
            transform += ` translate(${(Math.random()-.5)*intensity}px, ${(Math.random()-.5)*intensity}px)`;
        } else if (Game.shakeUntil) { Game.shakeUntil = 0; Game.shakeIntensity = 0; }
    ...
    updateSpellAnnouncement(caster, spellInfo, target) {
        ...
        let icon = spellInfo.icon ? `<div class="spell-slot" style="background-color:${spellInfo.color};background-image:url('${spellInfo.icon}');margin:4px auto;width:30px;height:30px;"></div>` : '';
        slot.innerHTML = `<span class="font-bold">${caster}</span>${icon}<span class="font-bold">${spellInfo.name}</span><span class="font-bold">${target ? ` sur ${target}` : ''}</span>`;
    ...
    triggerScreenShake(intensity = 'low') {
        const now = Date.now(); const duration = intensity === 'high' ? 600 : 250;
    ...
    resizeAllCanvases() {
        ...
        // On laisse Flexbox gérer la hauteur, on ajuste seulement la taille du canvas interne
        if (mainCanvas && mainCanvasCont) {
            mainCanvas.width = mainCanvasCont.clientWidth;
            mainCanvas.height = mainCanvasCont.clientHeight;
            Game.bubbleRadius = mainCanvas.width / (Config.GRID_COLS * 2 + 1) * 0.95;
        }
